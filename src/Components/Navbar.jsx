...
const Navbar = () => {
    ...
    return (
        <nav className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-950  fixed w-full top-0 z-50 shadow-sm shadow-gray-700">
                ...
                <div className="hidden md:flex space-x-6 font-medium">
                    {paths.map((path, idx) => (
                        <Link
                            key={path}
                            to={path}
                            className="hover:text-indigo-300 text-gray-200 transition-all duration-300"
                        >
                            {names[idx]}
                        </Link>
                    ))}
            ...
            {isOpen && (
                <div className="md:hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-950 px-6 py-4 space-y-3 shadow-md">
                    {paths.map((path, idx) => (
                        <Link
                            key={path}
                            to={path}
                            onClick={() => setIsOpen(false)}
                            className="block text-white hover:text-orange-400 transition-all duration-300 font-medium"
                        >
                            {names[idx]}
                        </Link>
                    ))}
